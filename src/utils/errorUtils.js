...
export const getFriendlyErrorMessage = (errorMsg) => {
  ...
  let message = typeof errorMsg === 'string' ? errorMsg : errorMsg?.message || String(errorMsg);

  // Trim whitespace
  message = message.trim();

  // 1. Verification Service / Decoder issues
  if (
  ...
  // 3. Technical System & Server Crash errors (Generic Database, Server status, JS Exceptions)
  if (
    ...
    /cannot (get|post|patch|put|delete)/i.test(message) ||
  ...
  // 4. Commodity delete blocked by active negotiations
  if (
